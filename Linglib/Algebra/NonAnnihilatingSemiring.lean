...
/-- A not-necessarily-annihilating semiring -/
class NonAnnihilatingSemiring (R : Type*) extends
    AddCommSemigroup R, Distrib R, Monoid R
...
/-- A not-necessarily-annihilating semiring
with commutative multiplication and idempotent addition -/
...
instance addSemigroup [AddSemigroup R] : AddSemigroup (WithAnnihilator R) where
  add_assoc := by
    intro A B C
    cases A <;> cases B <;> cases C <;> simp[←nonz_add, add_assoc]

instance addCommMagma [AddCommMagma R] : AddCommMagma (WithAnnihilator R) where
  add_comm := by
    intro A B
    cases A <;> cases B <;> simp[←nonz_add, add_comm]
...
instance semigroup [Semigroup R] : Semigroup (WithAnnihilator R) where
  mul_assoc := by
    intro A B C
    cases A <;> cases B <;> cases C <;> simp[←nonz_mul, mul_assoc]

instance commMagma [CommMagma R] : CommMagma (WithAnnihilator R) where
  mul_comm := by
    intro A B
    cases A <;> cases B <;> simp[←nonz_mul, mul_comm]
...
instance monoid [Monoid R] : Monoid (WithAnnihilator R) :=
  ...
  npow_succ := by
    intro n A
    cases n <;> cases A <;> simp[Option.map, none_eq_zero, some_eq_nonz, ←nonz_mul, pow_succ] }

instance distrib [Distrib R] : Distrib (WithAnnihilator R) where
  left_distrib := by
    intro A B C
    cases A <;> cases B <;> cases C <;> simp[←nonz_add, ←nonz_mul, left_distrib]
  right_distrib := by
    intro A B C
    cases A <;> cases B <;> cases C <;> simp[←nonz_add, ←nonz_mul, right_distrib]

/-- A semiring for `WithAnnihilator R`
given a non-annihilating semiring for `R` -/
instance semiring [NonAnnihilatingSemiring R] :
    Semiring (WithAnnihilator R) :=
  { addMonoid, addCommMagma, distrib, mulZeroClass, monoid with }
...
instance idemSemiring [self : NonAnnihilatingIdemSemiring R] :
  ...
  add_eq_sup := by
    intro A B
    cases A <;> cases B <;> simp[←nonz_add, ←nonz_sup, self.add_eq_sup] }

/-- An idempotent and commutative semiring for `WithAnnihilator R`
given a non-annihilating idempotent and commutative semiring for `R` -/
